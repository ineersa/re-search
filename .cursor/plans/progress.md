...
- **Run orchestration:** `RunOrchestratorInterface`, `RunOrchestrator` (stub)
- **Brief building:** `ResearchBriefBuilderInterface`, `ResearchBriefBuilder` (stub)
...
- **History loading:** `HistoryLoaderInterface`, `HistoryLoader`, `ResearchRunRepositoryInterface`, `ResearchRunRepository`
...
- **Service wiring:** Interface bindings in `config/services.yaml` (EventPublisher → NullEventPublisher, Repository → Doctrine-backed impl)
...
- **Persistence adapter:** `App\Research\Persistence\ResearchRunRepository` implements interface and maps entities to arrays for `HistoryLoader`
